function goMap() {
	...
	window.location.href = "map.html?longitude=" + longitude + "&latitude=" + latitude + "&direction=" + direction + "&positioningTime=" + positioningTime + "&city=" + city + "&key=" + encodeURIComponent(encodeURIComponent(key)) + "&type=" + type + "&citya=" + encodeURIComponent(encodeURIComponent(citya));
}
		...
		if(data) {
			...
			if (!data.result.datas || data.result.datas.length == 0) {
				...
				for(var i = 0; i < data.result.datas.length; i++) {
					...
					var finishStatus=data.result.datas[i].finishStatus;
					divResult += "<tr id=\"list" + i + "\"><td rowspan=\"" + pois.length + "\">" + (i+1) + "</td>";
					divResult += "<td rowspan=\"" + pois.length + "\">" + addTime + "</td>";
					divResult += "<td rowspan=\"" + pois.length + "\">" + finishStatus + "</td>";
					for(var j = 0 ; j < pois.length ; j++){
						...
						//divResult += "<td>" + (whither.source == 'amap.com' ? "高德" : whither.source == 'maps.baidu.com' ? "百度" : "话务员") + "&nbsp;</td>";
