...
dataSources=new Array("@TOT_POPU","@GRASS_POPU","@MINO_POPU","@IRR_RICE","@NOTPLANT","@PLANT_DRY","@PLANT_WATER","@AVE_REV","@AVE_RAIN","@CE_SHI1","@CE_SHI2","@CE_SHI3","@CE_SHI4","@CE_SHI5","");
...
function dhtmlXTreeObject(htmlObject, width, height, rootId){
    ...
    this.divCheck=true;
    //拖拽删除
    this.divCheckDel=false;
    
	this.id=htmlObject.id;
   ...
   this.selectChildNodes=new Array;
   //删除的节点
   ...
   //创建右键菜单
   this.menuObject =new dhtmlContextMenuObject();
...
/**
*   @desc: tree node constructor node实体
...
*   @topic: 0
*/
function dhtmlXTreeItemObject(itemId,itemText,parentObject,treeObject,actionHandler,mode,nodeType){
   this.fieldId=-1;
   this.xml="";
   this.htmlNode="";
   this.acolor="";
   ...
   this.checked=false;
	...
	//alert("nodeType="+nodeType+"  mode="+mode);
//是否选择
   ...
   dhtmlXTreeItemObject.prototype.asXML = function(){
   		this.xml='<tree id="0">';
   		this.writeXML(this);
   		this.xml+='</tree>'
   		return this.xml;
	...
	dhtmlXTreeItemObject.prototype.writeXML = function(obj){
		if(obj==0){
   			return;
   		}
   		if(obj==null){
		...
		if(childArray!=null){
			 childLen=obj.childNodes.length;
		}
		
   		
   		this.xml+='<item  id="'+obj.id+'" idLabel="'+obj.idLabel+'" newNode="'+obj.newNode+'" nodeType="'+obj.nodeType+'" parentID="'+obj.parentObject.id+ '" label="'+obj.label+'" text="'+obj.label+'" fieldId="'+obj.fieldId+'" open="true">';
   		for(var i=0;i<childLen;i++){
   			this.writeXML(childArray[i]);
   		}
   		this.xml+='</item>';
   };
   
    dhtmlXTreeItemObject.prototype.asTableXML = function(){
    	var n=0;
   		this.xml='<column name="'+this.label+'">';
   		//this.xml+='<column  label="地区" dataSource="@DIC" isNode="yes" typeName="" width="120"/>';;
   		//this.xml+='<column  label="年份" dataSource="@YEAR" isNode="yes" typeName="" width="120"/>';
   		var childArray=this.childNodes;
   		if(childArray!=null){
			 childLen=this.childNodes.length;
		}
		for(var i=0;i<childLen;i++){
   			this.writeTableXML(childArray[i],n);
   		}
   		this.xml+='</column>'
   		return this.xml;
   };
	...
	dhtmlXTreeItemObject.prototype.writeTableXML = function(obj,n){
		var childLen=0;
		var childArray=obj.childNodes;
		if(childArray!=null){
			 childLen=obj.childNodes.length;
		...
		var indexdata=dataSources.length;
   		if(n>=indexdata){
   			n=0;
   		}
   		this.xml+='<column   id="'+obj.id+'" label="'+obj.label+'" dataSource="'+dataSources[n]+'" isNode="'+booles+'" typeName="" width="120">';
   		for(var i=0;i<childLen;i++){
   			this.writeTableXML(childArray[i],n++);
   		}
   		this.xml+='</column>';
   ...
   dhtmlXTreeObject.prototype.asXML=function(node_id){
   		
   		var node=this._globalIdStorageFind(node_id);
   		if(node==null){
   			return "";
   		}
   		return node.asXML();
   ...
   dhtmlXTreeObject.prototype.getSelectedItemObject=function(itemId){
   		return this._globalIdStorageFind(itemId,1,1);
   }
/**
*     @desc: register node
*     @type: private
...
/**
*     @desc: return node object 当前tree的node是用数组存储的,不是数组，应该是一个数组对象。有些类似于Map
*     @param: itemId - node id
*     @type: private
*     @topic: 3
*/
...
*     @topic: 2
*/
   dhtmlXTreeObject.prototype._attachChildNode=function(parentObject,itemId,itemText,itemActionHandler,image1,image2,image3,optionStr,childs,beforeNode,afterNode,nodeType){
		...
		if (!(n=this._idpull[itemId]) || n.span!=-1){
         	n=Nodes[Count]=new dhtmlXTreeItemObject(itemId,itemText,parentObject,this,itemActionHandler,1,nodeType);
...
*     @topic: 2  
*/
   dhtmlXTreeObject.prototype.addNewItem=function(parentId,itemId,itemText,nodeType,itemActionHandler,image1,image2,image3,optionStr,children){
      var parentObject=this._globalIdStorageFind(parentId);
      if (!parentObject) return (-1);
      var nodez=this._attachChildNode(parentObject,itemId,itemText,itemActionHandler,image1,image2,image3,optionStr,children,null,null,nodeType);
    	          ...
    	          if (this.waitUpdateXML){
				  		if (this._globalIdStorageFind(a.id))
		                    ...
		                    var newNode=this._attachChildNode(temp,a.id,a.text,0,a.im0,a.im1,a.im2,zST.join(","),a.child,0,preNode);
		                     newNode.fieldId=a.fieldId;
		                     newNode.nodeType=a.nodeType;
		                     newNode.text=a.label;
		                     if(a.selected){
		                     	 newNode.isCalalog=1;
		                    	 this._selectNodeItems(newNode.id,"Red");
...
/**  
*     @desc: set correct tree-line and node images //修正正确的tree的链接和图片
*     @type: private
*     @param: itemObject - item object
*     @topic: 6  
*/
...
/**
*     @desc: set correct tree-line images 设置正确的链接
*     @type: private
*     @param: itemObject - item object
*     @topic: 6
*/
...
*/   
   ...
   dhtmlXTreeObject.prototype.delSelItemTable=function(){
	   	for(var i=0;i<this.delNodes.length;i++){
	   		var dragNode=this.delNodes[i];
	   		this.deleteItem(dragNode.id);
	   	}
   };   
 /**
...
*     @topic: 0
*/
   dhtmlXTreeObject.prototype._createItemTable=function(aTable,itemObject,mode){
   				var childArray=itemObject.childNodes;
   				if(itemObject!=0 && !itemObject.checked && itemObject.fieldId!=-1 && itemObject.fieldId!=-99){
	 				var tr=document.createElement("tr");
				...
				//	var input=document.createElement("input");
					td_2.innerHTML=itemObject.label;
					td_2.width="40%";
					tr.appendChild(td_1);
					td_1.innerHTML=itemObject.text;
					td_1.width="40%";
					tr.appendChild(td_2);
				//	td_2.innerHTML=itemObject.id;
				//	tr.appendChild(td_3);
				//	td_2.appendChild(input);
				//	td_3.innerHTML=itemObject.id;
					var checkbox=document.createElement("input");
					checkbox.type="checkbox";
					checkbox.value=itemObject.fieldId;
					checkbox.name="fileds";
					checkbox.style.display="none";
					td_2.appendChild(checkbox);
				//	td_3.appendChild(checkbox);
					
					checkbox.checked="true";
					itemObject.checked=true;
					
					this.setItemColor(itemObject.id,"Red");
					//
					tr.appendChild(td_4);
					var button=document.createElement("input");
					button.noedObject=itemObject;
					button.trHTML=tr;
					button.treeNode=this;
					button.type="button";
					button.value="删除";
					button.onclick=function(){
						button.noedObject.checked=false;
						button.treeNode.setItemColor(button.noedObject.id,"black");
						aTable.removeChild(tr);
					}
					td_4.appendChild(button);
				}
				this._selectItemTable(childArray,aTable)
	
   };
    dhtmlXTreeObject.prototype._selectItemTable=function(childArray,aTable){
    	if(childArray==null){
    		return;
    	}
    	...
    	for(var i=0;i<len;i++){
    		node=childArray[i];
    		if(node!=0 && !node.checked && node.fieldId!=-1 && node.fieldId!=-99){
    				var tr=document.createElement("tr");
	 				aTable.appendChild(tr)
					var td_1=document.createElement("td");
					var td_2=document.createElement("td");
				//	var td_3=document.createElement("td");
					var td_4=document.createElement("td");
				//	var input=document.createElement("input");
					td_2.innerHTML=node.label;
					td_2.width="40%";
					tr.appendChild(td_1);
					td_1.innerHTML=node.text;
					td_1.width="40%";
					tr.appendChild(td_2);
				//	td_2.innerHTML=node.id;
				//	tr.appendChild(td_3);
				//	td_2.appendChild(input);
				//	td_3.innerHTML=node.id;
					var checkbox=document.createElement("input");
					checkbox.type="checkbox";
					checkbox.value=node.fieldId;
					checkbox.name="fileds";
					
					checkbox.style.display="none";
					td_2.appendChild(checkbox);
					checkbox.checked="true";
					this.setItemColor(node.id,"Red");
					node.checked=true;
					//
					tr.appendChild(td_4);
					var button=document.createElement("input");
					button.noedObject=node;
					button.trHTML=tr;
					button.type="button";
					button.value="删除";
					button.treeNode=this;
					button.onclick=function(){
						var e=window.event;
						var buttonObject=e.srcElement;
						buttonObject.noedObject.checked=false;
						buttonObject.treeNode.setItemColor(buttonObject.noedObject.id,"black");
						aTable.removeChild(buttonObject.trHTML);
					}
					td_4.appendChild(button);
    	}
    		this._selectItemTable(node.childNodes,aTable);
    	}
    }
...
//      itemObject.span.appendChild(document.createTextNode(itemObject.label));
	  ...
	  if(this.menuBoolean){
	  	this.menuObject.addContextMenuItem(td2,this);
	  }
...
dhtmlXTreeObject.prototype._recreateBranch=function(itemObject,targetObject,beforeNode,level){
    var i; var st="";
     ...
     if(!itemObject.checkedTree){
	   ...
	   //需要重新定义id
	   newNode=this._attachChildNode(targetObject,itemObject.id,itemObject.label,0,itemObject.images[0],itemObject.images[1],itemObject.images[2],st,0,beforeNode);
	   //copy user data
	   ...
	   //补充没有完整的数据
	   newNode.fieldId=itemObject.fieldId;
	   if(targetObject.nodeType==1){
		   newNode.nodeType=2;
	   }
	   if (t2){
...
// 可以复制
  // if ((framesMove)||(oldTree.dpcpy)) {//interframe drag flag
      ...
      //移动到固定位置
      if(itemObject.treeNod.divCheckDel){
			...
			if(oldTree.divName==targetObject.treeNod.divName){
	...
	//      if (z.childsCount==0)
      ...
      //具体就在这里实现,这是只是实现了删除
        if (!oldTree.dpcpy){
        	oldTree._clearSelectStyle(_otiid,targetObject.treeNod.selectChildNodes);
         	oldTree.deleteItem(_otiid);
         }else{
         	oldTree._selectNodeItems(_otiid,"Red");
         }
...
dhtmlXTreeObject.prototype.enableSefDarg = function (mode) {
	this.divName = mode;
};

dhtmlXTreeObject.prototype.enableDivCheck = function (mode) {
	this.divCheck = mode;
};
dhtmlXTreeObject.prototype.enableDivCheckDel = function (mode) {
	this.divCheckDel = mode;
};

dhtmlXTreeObject.prototype.enableMenuBoolean = function (mode) {
	this.menuBoolean = mode;
};

//清除选择的样式和状态
dhtmlXTreeObject.prototype._clearSelectStyle=function(itemId,oldNodeArray){
      var sNode=this._globalIdStorageFind(itemId);
      if (!sNode) return;
      this._clearNodesStyle(itemId,sNode,oldNodeArray);
      
   };



dhtmlXTreeObject.prototype._clearNodesStyle=function(itemId,htmlObject,oldNodeArray){
   if ((!htmlObject)||(!htmlObject.parentObject)) return 0;
   		if((!oldNodeArray)){
   			return 0;
   		}
   	  for(var n=0;n<oldNodeArray.length;n++){
   	 	var o=oldNodeArray[n];
   	  	if(o.id==itemId){
   	  		o.treeNod.setItemColor(itemId,"navy");
   	  		o.checkedTree=false;
   	  		break;
   	  	}
   	  }
      ...
      var Nodes=sN.childNodes;
            for (var i=0; i<Count; i++)
            {
               	this._clearNodesStyle(Nodes[i].id,Nodes[i],oldNodeArray)
            }
   };


dhtmlXTreeObject.prototype._selectNodeItems=function(itemId){
      var sNode=this._globalIdStorageFind(itemId);
      if (!sNode) return;
      
      this._selectNodes(itemId,sNode);
      
   };

// 根据父id得到子id对象

dhtmlXTreeObject.prototype._selectNodes=function(itemId,htmlObject){
   if ((!htmlObject)||(!htmlObject.parentObject)) return 0;
   
   	  this.setItemColor(itemId,"Red");
   	  this.selectChildNodes.push(htmlObject);
   	  htmlObject.checkedTree=true;
      var sN=htmlObject;
      var Count=sN.childsCount;
      var Nodes=sN.childNodes;
            ...
            for (var i=0; i<Count; i++)
            {
               	this._selectNodes(Nodes[i].id,Nodes[i])
            }
   };
...
*/
   ...
   /**
*     @desc: clear selection from node 清除选择
...
dhtmlXTreeObject.prototype._dragIn1=function(htmlObject,x,y){
