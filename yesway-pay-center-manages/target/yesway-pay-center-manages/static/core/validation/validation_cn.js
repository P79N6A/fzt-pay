...
Validator.messageSource['zh-cn'] = [
	...
	['max-bits', '最大字节数为%s,当前字节数为%s.提示：汉字为两个字节，数字或字符为一个字节'],//2012-08-09 wangjunx add 
...
Validator.prototype = {
	...
	error : function(v,elm,useTitle) {
		...
		if(typeof error == 'string') {
			//2012-08-09 wangjunx add begin
			if(this.className=='max-bits'){
			    //if(v) args.push(v.replace(/[^x00-\xff]/g,'xx').length);//alert(v.replace(/[^x00-\xff]/g,'xx').length);
				if(v) args.push(v.getBytesLength());
			}
			else{
				if(v) args.push(v.length);
			}
			//2012-08-09 wangjunx add end
...
Validation.addAllThese([
	...
	//2012-08-09 wangjunx add begin
	['max-bits',function(v,elm,args,metadata) {//alert(v.replace(/[^x00-\xff]/g,'xx').length);alert(parseInt(args[1]));
		return v.getBytesLength() <= parseInt(args[0]);
	}],
...
String.prototype.getBytesLengths = function() {   
    var totalLength = 0;     
    var charCode;  
    for (var i = 0; i < this.length; i++) {  
        charCode = this.charCodeAt(i);  
