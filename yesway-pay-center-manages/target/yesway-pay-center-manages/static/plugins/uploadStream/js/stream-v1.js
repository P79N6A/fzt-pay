...
(function(){
	var Provider, nIdCount = 0, aOtherBrowsers = ["Maxthon", "SE 2.X", "QQBrowser"],
		...
		sCellFileTemplate = '<b class="stream-uploading-ico"></b>' +
					...
					'	<a class="stream-stoporstart" href="javascript:void(0)">\u6682\u505C</a>' +
					'	<a class="stream-cancel" href="javascript:void(0)">\u5220\u9664</a>' +
					'</div>' +
					'<div class="stream-cell-infos">' +
					'	<span class="stream-cell-info" style="width:110px;">\u901f\u5ea6\uff1a<em class="stream-speed"></em></span>' +
					'	<span class="stream-cell-info" style="width:181px;">\u5df2\u4e0a\u4f20\uff1a<em class="stream-uploaded"></em></span>' +
					'	<span class="stream-cell-info" style="margin-left:-17px;">\u5269\u4f59\u65f6\u95f4\uff1a<em class="stream-remain-time"></em></span>' +
					'</div>',
	...
	function fShowMessage(msg,msgID,warning) {
		var o = document.getElementById(msgID);
		o&&(o.innerHTML=msg);
	}
	...
	StreamProvider.prototype = {
			...
			switch (evt.type) {
				...
				case "drop" :
					var callback = function(files, self) {
						...
						0 < list.length && self.fire("fileselect", {fileList:list,fileAll:files});
					};
					if (bFolder) {
		...
		traverseFileTree : function (directory, callback) {
			...
			reader.readEntries(function(entries) {
				callback.pending--;
				if (!entries.length) {
					fShowMessage("\u5FFD\u7565\u7A7A\u6587\u4EF6\u5939\uFF1A`" + relativePath + directory.name + "`",this.config.messagerId,true);
				} else {
		...
		updateFileList : function(a) {
			...
			var fileselectInfos = {};
			0 < b.length && this.fire("fileselect", {fileList:b,fileAll:a});
	...
	Main.prototype = {
		...
		addStreamTask : function(a) {
			...
			if (!this.config.customered) {
				...
				if(!this.config.multipleFiles)this.containerPanel.innerHTML="";
		...
		renderUI : function(file_id) {
			...
			if(this.config.multipleFiles)this.getNode("_stream-total-size", this.totalContainerPanel).innerHTML = _total;
		},
		bindUI : function(file_id) {
			var b = this.uploadInfo[file_id].progressNode, cancelBtn = this.getNode("stream-cancel", b);
			var stopOrConBtn = this.getNode("stream-stoporstart", b);
			this.cancelBtnHandler = fExtend(this.cancelUploadHandler, this, {type : "click",	nodeId : file_id});
			this.stopOrConBtnHandler = fExtend(this.stopOrContinueHandler, this, {type : "click",	nodeId : file_id});
			fAddEventListener(cancelBtn, "click", this.cancelBtnHandler);
			fAddEventListener(stopOrConBtn, "click", this.stopOrConBtnHandler);
		},
		...
		onSelect : function(list) {
			fShowMessage("selected files: " + list.length,this.config.messagerId);
		},
		onFileCountExceed : function(selected, limit) {
			fShowMessage("File counts:" + selected + ", but limited:" + limit,this.config.messagerId,true);
		},
		onMaxSizeExceed : function(file) {
			fShowMessage("File:" + file.name + " size is:" + file.size +" Exceed limited:" + file.limitSize,this.config.messagerId,true);
		},
		onExtNameMismatch: function(file) {
			fShowMessage("Allow ext name: [" + file.filters.toString() + "], not for " + file.name,this.config.messagerId,true);
		},
		onAddTask: function(file) {
			fShowMessage("Add to task << name: [" + file.name,this.config.messagerId);
		},
		onGetImg:function(img){
			
		},
		onCancel : function(file) {
			fShowMessage("Canceled: " + file.name,this.config.messagerId);
		},
		onStop : function() {
			fShowMessage("Stopped!",this.config.messagerId);
		},
		onCancelAll : function(numbers) {
			fShowMessage(numbers + " files Canceled! ",this.config.messagerId);
		},
		onComplete : function(file) {
			fShowMessage("File:" + file.name + ", Size:" + file.size + " onComplete	[OK]",this.config.messagerId);
		},
		onQueueComplete : function(msg) {
			fShowMessage("onQueueComplete(msg: "+msg+")	---==>		[OK]",this.config.messagerId);
		},
		onUploadError : function(status, msg) {
			fShowMessage("Error Occur.  Status:" + status + ", Message: " + msg,this.config.messagerId,true);
		},
		disable : function() {
		...
		cancelOne : function(file_id, stopping) {
			...
			if (actived) {
				this.uploading = !1;
				if(this.config.multipleFiles)this.upload();
			}
			if(!this.config.multipleFiles)return true;
		...
		stopOrContinueHandler:function(event,b){
			var c = event || window.event, id = b.nodeId, self = this;
			this.preventDefault(c);
			this.stopPropagation(c);
			if(this.uploading){
				this.getNode("stream-stoporstart", this.uploadInfo[id].progressNode).innerHTML="\u7EE7\u7EED";
				this.stop();
			}else{
				this.getNode("stream-stoporstart", this.uploadInfo[id].progressNode).innerHTML="\u6682\u505C";
				this.upload();
			}
		},
			...
			xhr.onreadystatechange = function() {
				...
				try {
					...
					if (token) {
						...
						fShowMessage(msg,this.config.messagerId,true);
					}
				} catch(e) {
		...
		uploadComplete : function(a) {
			...
			if (!this.config.customered) {
				...
				this.getNode("stream-stoporstart", progressNode).innerHTML = "";
		...
		fileSelect : function(a) {
			var localFiles = a.fileAll;
			if(this.get("onImgGet"))fShowMessage("(上传图片尺寸:180px*180px，图片格式：支持jpg、png)",this.config.messagerId);
			var a = a.fileList, b = 0, c, files = [];
			for (c = 0; c < a.length; c++)
				files.push(a[c].config);
			var selectRe = this.get("onSelect") ? this.get("onSelect")(files) : this.onSelect(files);
			if(selectRe) return false;
			...
			for (c = 0; c < a.length; c++)
				this.validateFile(a[c]) && (this.fileProvider instanceof StreamProvider && this.get("onImgGet") ? this.get("onImgGet")(localFiles,this.addStreamTask,a[c],this) : this.addStreamTask(a[c]));
		},
