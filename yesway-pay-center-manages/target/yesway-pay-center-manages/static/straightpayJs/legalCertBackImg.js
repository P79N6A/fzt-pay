...
var legalCertBackImgConfig = {
	browseFileId : "legalCertBackImgButton",
	browseFileBtn : " ",
	dragAndDropArea : "legalCertBackImgButton",
	dragAndDropTips : "<span></span>",
	filesQueueId : "legalCertBackImgContainer",
	filesQueueHeight : 80,
	messagerId : "legalCertBackImgMessage",
	multipleFiles : false,
...
//				$("#legalCertBackImgMessage").text("上传尺寸不对");
//				$("#legalCertBackImgButton").html("");
//				uploadAddInfo.isUploadImg = false;
//				uploadUpdateInfo.isUploadImg = false;
//			} else {
				$("#legalCertBackImgContainer").show();
...
//			}
	...
	onExtNameMismatch : function(name, filters) {
		$("#legalCertBackImgMessage").text("上传格式不对");
		$("#legalCertBackImgButton").html("");
		uploadAddInfo.isUploadImg = false;
		uploadUpdateInfo.isUploadImg = false;
	},
	...
	onComplete : function(file) {
		...
		console.log("message:"+eval("(" + file.msg + ")").message);
		console.log("message_two:"+eval("(" + file.msg + ")").message_two);

		$("#legalCertBackImgContainer").hide();
		document.getElementById("legalCertBackImgMessage").innerHTML = "上传成功!";
		document.getElementById("legalCertBackImgButton").innerHTML = "<img id='imgSrc' src='"
				+ serverPath + eval("(" + file.msg + ")").message + "'>";
		$("#legal_cert_back_image_url").val(serverPath + eval("(" + file.msg + ")").message);
		$("#legal_cert_back_image").val(eval("(" + file.msg + ")").imageId);
	...
	onUploadError : function(status, msg) {
		$("#legalCertBackImgMessage").text("上传失败");
        $("#legalCertBackImgButton").html("");
		uploadAddInfo.isUploadImg = false;
		uploadUpdateInfo.isUploadImg = false;
	},
