...
//	private String BUTTON_CODE ;
		...
		try {
			...
			if(resultList.size()>0){
				...
				if(payToolType==1){//支付宝
					String YESWAY_PRIVATE_KEY="";//九五智驾的RSA加密私钥
					String ALIPAY_PUBLIC_KEY="";//支付宝的RSA加密公钥
					String x="YESWAY";
					String y="ALIPAY";
					String z="RSA_PRIVATE";
					String w="RSA_PUBLIC";
					Credential credential=credentialService.getCredential(x, y, z);
					if(credential==null){
						logger.info("\n 1查询证书表为null");
					}else{
						YESWAY_PRIVATE_KEY=credential.getCredential();
					}
					Credential data2=credentialService.getCredential(y, x, w);
					if(data2==null){
						logger.info("\n 2查询证书表为null");
					}else{
						ALIPAY_PUBLIC_KEY=data2.getCredential();
					}
					AlipayClient alipayClient = new DefaultAlipayClient(ALIPAY_GATEWAY,ALIPAY_APPID,YESWAY_PRIVATE_KEY,"JSON",Contants.CHARSET,ALIPAY_PUBLIC_KEY,Contants.SIGN_TYPE);
					...
					//String op_user_id = OP_USER_ID;// 操作员帐号,
					...
					String credentialAdress="";
					try {
						logger.info("调用微信退款接口:"+wxUrl);
						weixinPost = ClientCustomSSL.doRefund(wxUrl, xmlStr,WEIXIN_MCHID,credentialAdress).toString();
					...
					if(BUTTON_CODE.equals("ON")){//代表开
						...
						if(resultCode.equals(CODE_FAIL) && errCode.equals(ERRCODE)){
							...
							params2.put("mch_id", WEIXIN_MCHID);		
							...
							// 发送请求并接收响应数据
							logger.info("调用微信退款接口:"+wxUrl);
							weixinPost2 = ClientCustomSSL.doRefund(wxUrl, xmlStr2,WEIXIN_MCHID,credentialAdress).toString();
					...
					logger.info("未知的支付工具类型");
				}
			}else{//是否存在该订单IF
				// 查询失败，直接返回错误信息给请求端
				logger.info("无法查询到该订单号对应的订单信息：商户订单号[" + outTradeNo + "]");
				MessageHeader resultMsgHeader = new MessageHeader();
	...
	public Map<String, Object> refundquery(HttpServletRequest request,HttpServletResponse response) {
		...
		try {
			String appId = WEIXIN_APPID;// 微信开放平台审核通过的应用APPID
...
//				   AlipayTradeRefundRequest requestQuery = new AlipayTradeRefundRequest();
					Map<String,Object> param = new HashMap<String,Object>();
					param.put("out_trade_no", payOrderNo);
	 ...
	 * 解决有时生成的签名长度为31位
	 */
	public Map<String, Object> getSignStr(Map<String, Object> params){
		int num = 0;
		String sign = "";
		//为了避免有时生成的签名长度为31位
