...
public class ProtocolAuthV1Interceptor implements HandlerInterceptor {
...
//		logger.info("request.setAttribute(\"messageheader\", "+messageheader+")");
    	...
    	if(requestUri.contains("/pay/orderquery") || requestUri.contains("/pay/closeorder") 
			...
			|| requestUri.contains("/pay/v1/orderquery") || requestUri.contains("/pay/v1/closeorder") 
    			|| requestUri.contains("/pay/v1/refundquery") || requestUri.contains("/pay/v1/weixin/unifiedorder") || requestUri.contains("/pay/v1/alipayrefundquery")
				){
		...
		//查询具体的商户是否配置支付账号
		List<PayCenterConfiguration> payCenterConfigs = payCenterConfigurationService.findList(queryPayCenterConfig);
		
		//如果商户没有配置自己的账号，则使用车厂自己的账号
		if(payCenterConfigs == null || payCenterConfigs.size() == 0){
			queryPayCenterConfig.setMchId("all");
			payCenterConfigs = payCenterConfigurationService.findList(queryPayCenterConfig);
		}
		
		if(payCenterConfigs == null || payCenterConfigs.size() == 0){
			logger.error(">>>商户信息配置错误",new RequestParamException());
			ResponseMessageHearder.responseMessageHearderError(response, new RequestParamException());
            return false;
		}
		
		PayCenterConfiguration payCenterConfig = payCenterConfigs.get(0);
		if(StringUtils.isBlank(payCenterConfig.getAesKey()) || StringUtils.isBlank(payCenterConfig.getYeswayPrivateKey())
				|| StringUtils.isBlank(payCenterConfig.getYeswayPublicKey())){
			logger.error("商户配置信息错误！");
			ResponseMessageHearder.responseMessageHearderError(response, new RequestParamException());
            return false;
		}
...
//		Credential rsaCredential = credentialService.getCredential(FROM,hOemId, SignTypeEnum.RSA_PRIVATE.name());	
		//对支付单号处理(下单和退款)
		if(requestUri.contains("/pay/unifiedorder") || requestUri.contains("/pay/v1/unifiedorder")){
			String payOrderNoSign = messageJSON.getString("payOrderNoSign");
			String payOrderNo = null;
			try {
				if(payOrderNoSign !=null){
					payOrderNo = RSAUtils.decryptByPrivateKey(payOrderNoSign,payCenterConfig.getYeswayPrivateKey());
				}
				logger.info("解密数据payOrderNo是 "+payOrderNo);
			} catch (Exception e) {
				e.printStackTrace();
				logger.error("解密数据出错" + e.getMessage());
			}
    		request.setAttribute("payOrderNo", payOrderNo);			
    	}
		if(requestUri.contains("/pay/refund") || requestUri.contains("/pay/v1/refund")){
			String payOrderNoSign = messageJSON.getString("payOrderNoSign");
			...
			try {
				...
				if(payRefundNoSign !=null){
					payRefundNo = RSAUtils.decryptByPrivateKey(payRefundNoSign,payCenterConfig.getYeswayPrivateKey());
				}
				logger.info("解密数据payOrderNo是 "+payOrderNo);
			} catch (Exception e) {
				e.printStackTrace();
        ...
        String aesSecretKey = payCenterConfig.getAesKey();
        try {
        	//解密
        	sdata =SecurityUtil_AES.decrypt(sdata, aesSecretKey);
        	logger.info("解密数据是"+sdata);
		...
		//解析签名数据
        String rsaPrivateKey = payCenterConfig.getYeswayPrivateKey();
		...
		try {
			...
			logger.info(data);
			logger.info("sdigest===="+sdigest);
			rDigest = SHADigestUtils.encryptSHA(data);
			logger.info("rDigest===="+rDigest);
		} catch (Exception e) {
